...
int *vocab_hash;
long long vocab_max_size = 1000, vocab_size = 0, layer1_size = 100, word_vec_size = 50, ad_id_vec_size = 30, ader_id_vec_size = 20;
long long train_words = 0, word_count_actual = 0, file_size = 0, classes = 0;
long long ad_table_size = 0, ader_table_size = 0;
real alpha = 0.025, starting_alpha, sample = 0;
real *word_vec, *syn1, *weight, *expTable;
real *ad_id_vec, *ader_id_vec;
clock_t start;

struct vocab_word_cnt* word_table;
vector<vector<int> > title_tokens, desc_tokens, keyword_tokens;
char keyword_token_file[MAX_STRING], title_token_file[MAX_STRING], desc_token_file[MAX_STRING];
char ader_id_map_path[MAX_STRING], ad_id_map_path[MAX_STRING];
unordered_map<int, int> ader_id_map, ad_id_map;
long max_token_id;
...
void InitNet() {
    ...
    a = posix_memalign((void **)&ader_id_vec, 128, (long long)ader_table_size * ader_id_vec_size * sizeof(real));
    if (ader_id_vec == NULL) {printf("Memory allocation failed for ader_id_vec\n"); exit(1);}
    ...
    for (b = 0; b < ader_id_vec_size; b++) for (a = 0; a < ader_table_size; a++)
        ader_id_vec[a * ader_id_vec_size + b] = (rand() / (real)RAND_MAX - 0.5) / ader_id_vec_size;
    // CreateBinaryTree();
}
...
bool ReadTrainRecord(FILE* fin, train_record* record) {
  ...
  record->ad_id = ad_id_map[ad_id];
  record->ader_id = ader_id_map[ader_id];
  return true;
}
...
void *TrainModelThread(void *id) {
    ...
    long long ad_id_start, ader_id_start;
    ...
    int word_vector_end = ader_id_vec_size + ad_id_vec_size + word_vec_size;
    ...
    while (1) {
        ...
        if (sentence_length == 0) {
            if (last_status == DESC) {
              ...
              last_status = KEY_WORD;
            } else if (last_status == KEY_WORD) {
              last_status = TITLE;
            } else if (last_status == TITLE) {
              last_status = DESC;
            }
            
            vector<int>* token_list = NULL;
            switch (last_status) {
              case KEY_WORD: token_list = &keyword_tokens[cur_record.keyword_id];break; 
              case DESC    : token_list = &desc_tokens[cur_record.desc_id];break;
              case TITLE   : token_list = &title_tokens[cur_record.title_id];break;
            }
            size_t i = 0;
            while (1) {
                int id;                
        ...
        if (cbow) {  //train the cbow architecture
            ...
            for (a = b; a < window * 2 + 1 - b; a++) if (a != window) {
                ...
                if (negative > 0) for (d = 0; d < negative + 1; d++) {
                    ...
                    for (c = 0; c < ad_id_vec_size; c++) f += ad_id_vec[c + ad_id_start] * weight[c + l2 + ader_id_vector_end];
                    for (c = 0; c < word_vec_size; c++) f += word_vec[c + l1] * weight[c + l2 + ad_id_vector_end];
                    if (f > MAX_EXP) g = (label - 1) * alpha;
                    else if (f < -MAX_EXP) g = (label - 0) * alpha;
                    else g = (label - expTable[(int)((f + MAX_EXP) * (EXP_TABLE_SIZE / MAX_EXP / 2))]) * alpha;
                    for (c = 0; c < layer1_size; c++) neu1e[c] += g * weight[c + l2];
                    ...
                    for (c = 0; c < ad_id_vec_size; c++) weight[c + l2 + ader_id_vector_end] += g * ad_id_vec[c + ad_id_start];
                    for (c = 0; c < word_vec_size ; c++) weight[c + l2 + ad_id_vector_end] += g * word_vec[c + l1];
                }
                ...
                for (c = 0 ; c < word_vec_size; c++) word_vec[c + l1] += neu1e[c + ad_id_vector_end];
            }
        }
        sentence_position++;
...
void output(const char * output_path) {
  ...
  fprintf(output_file,"%lld %lld\n", ader_table_size, ader_id_vec_size);
  for (int i = 0; i < ader_table_size; i++) {
    for (int j = 0; j < ader_id_vec_size; j++) {
      fprintf(output_file, "%f ", ader_id_vec[i * ader_id_vec_size + j]);
    }
    fprintf(output_file, "\n");
  }
  ...
  fprintf(output_file, "%lld %lld\n", ad_table_size, ad_id_vec_size);
  for (int i = 0; i < ad_table_size; i++) {
    for (int j = 0; j < ad_id_vec_size; j++) {
      fprintf(output_file, "%f ", ad_id_vec[i * ad_id_vec_size + j]);
    }
    fprintf(output_file, "\n");
  }
  ...
  string word_output_path = output_path;
  word_output_path += ".word";

  FILE* word_output_file = fopen(word_output_path.c_str(), "wb");  

  printf("word");
  fprintf(word_output_file, "%lld %lld\n", vocab_size, word_vec_size);
  for (int i = 0; i < vocab_size; i++) {
    for (int j = 0; j < word_vec_size; j++) {
      fprintf(word_output_file, "%f ", word_vec[i * word_vec_size + j]);
    }
    fprintf(word_output_file, "\n");
  }
  fclose(word_output_file);
}
...
void TrainModel() {
    ...
    ReadIdMap(ader_id_map_path, &ader_id_map);
    ader_table_size = ader_id_map.size();

    ReadIdMap(ad_id_map_path, &ad_id_map);
    ad_table_size = ad_id_map.size();

    //Read token
    ReadTokenList(keyword_token_file, &keyword_tokens);
    ReadTokenList(title_token_file, &title_tokens);
    ReadTokenList(desc_token_file, &desc_tokens);
